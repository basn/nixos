...
The configured audit profile's `cron/output` directory is explicitly mounted
inside its Docker terminal sandbox at `/cron-output`, read-only. Other profiles
are not mounted automatically, and the rest of `.hermes` (credentials, session
databases, and other state) remains unmounted. For example, the latest saved
report for an audit-profile job can be inspected with:
